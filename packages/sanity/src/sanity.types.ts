...
// Query: *[_type == "blog" && (seoHideFromLists != true) && (count($categories) == 0 || count(categories[_ref in *[_type=="category" && slug.current in $categories]._id]) > 0)] | order(orderRank asc) [$start...$end]{      _type,  _id,  title,  description,  "slug":slug.current,  orderRank,    image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  },  publishedAt,    authors[0]->{    _id,    name,    position,      image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  }  }, "categories": categories[]->{    title,    "slug": slug.current  }  }
...
// Query: *[_type == "author"] | order(name asc) {    _id,    name  }
...
// Source: ../../packages/sanity/src/query.ts
// Variable: queryAvailableCategories
// Query: *[_type == "category" && _id in *[_type == "blog" && (seoHideFromLists != true) && (count($categories) == 0 || count(categories[_ref in *[_type=="category" && slug.current in $categories]._id]) > 0)].categories[]._ref] | order(title asc) {    _id,    title,    "slug": slug.current  }
export type QueryAvailableCategoriesResult = Array<{
  _id: string;
  title: string;
  slug: string;
}>;
